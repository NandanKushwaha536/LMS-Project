  ...
  return (
    <HomeLayout>
        <div className="flex flex-col gap-10 items-center justify-center min-h[90vh] py-10 text-white mx-5">
            <div className='text-center text-2xl font-extrabold text-yellow-500'>
                 ...
                 lectures.map((lecture, idx)=>{
                     return(
                         <li className='space-y-2' key={lecture._id}>
                             <p className='cuser-pointer' onClick={()=> setCurrentVideo(idx)}>
                                 <span>
                                     {" "} Lectrue {idx +1} : {' '}
                                 </span>
                                 {lecture?.title}
                             ...
                             {role === "ADMIN" && (
                                 <button onClick={()=> onLectureDelete(state?._id, lecture?._id)} className='btn-primary px-2 py-1 rounded-md font-semibold text-sm'>
                                    Delete lecture
                                 </button>
                             )}
                         </li>
